...
$(function()
{
    ...
    if($('#photos').length)
    {
        $('#photos').sortable(
        {
            ...
            {
                var _photos_data = $('#photos').sortable('serialize');
                $.ajax({data:_photos_data, type:'post', url:'/back/show/'+$('#photos').data('show-id')+'/gallery?action=resort'});
                notice('Позиция фото успешно изменена');
            }
        });
    }
    ...
    {
        $('.add-description-tpl').on('click', function() {
            let _box = $('.description-box');
            let _tpl = $('.description-tpl').html();
            _box.append(_tpl);
    ...
    if($('.quill-form').length)
    {
        // $(document).ready(function() {
            $('.quill-form').on('submit', function () {
                $(this).append("<textarea name='content' style='display:none'>"+$('.ql-editor').html()+"</textarea>");
            });
        // });
    }

    if($('#photos_peoples').length)
    {
        $('#photos_peoples').sortable(
        {
            update: function (event, ui)
            {
                var _photos_data = $('#photos_peoples').sortable('serialize');
                $.ajax({data:_photos_data, type:'post', url:'/back/actors/peoples?edit='+$('#photos_peoples').data('people-id')+'&action=resort'});
                notice('Позиция фото успешно изменена');
            }
        });
    }
...
function setcookie(name, value, ttl)
{
	...
	document.cookie = name + '=' + value + expires + '; path=/; domain=.' + window.location.hostname + '; secure';
}
